...
# ✅ 新增：根据每个分类目录下最新图片，更新 index.html 首页封面图（img src）
def update_index_covers():
    index_file = "index.html"
    if not os.path.exists(index_file):
        print("❌ index.html 不存在，跳过封面图更新")
        return

    # 找到所有分类目录
    category_dirs = [d for d in os.listdir() if os.path.isdir(d) and not d.startswith('.') and d.lower() not in ['images', 'generator', 'keywords']]
    category_to_img = {}
    for folder in category_dirs:
        ...
        category_to_img[folder] = f"{folder}/{images[0]}"

    # 读取 index.html 并替换 <img src="xxx"> 路径
    with open(index_file, 'r', encoding='utf-8') as f:
    ...
    if changed:
        with open(index_file, 'w', encoding='utf-8') as f:
            f.write(str(soup))
        print("✅ 首页封面图已更新为每个分类的最新图片")
    else:
        print("⚠️ 没有需要替换的 <img> 标签，封面图未更新")
